...
export default function MyOrders() {
  ...
  const ratingMutation = useMutation(
    ({ orderId, rating, comment }) => api.post('/orders/'+orderId+'/rate', { rating, comment }),
    {
      onSuccess: () => {
  ...
  return (
            ...
            <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
              ...
              {o.status === 'shipped' && (
                <button onClick={() => setConfirmOrder(o)}
                  style={{ background: C.eco, color: '#fff', border: 'none', padding: '0.6rem 1.2rem', borderRadius: 100, fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                  ✅ Confirmer réception
                </button>
              )}
              {o.status === 'delivered' && !o.seller_rating && (
                <button onClick={() => setRatingOrder(o)}
                  style={{ background:'linear-gradient(135deg,#F59E0B,#D97706)', color:'#fff', border:'none', padding:'0.6rem 1.2rem', borderRadius:100, fontSize:'0.82rem', fontWeight:600, cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
                  ⭐ Noter le vendeur
                </button>
              )}
              {o.status === 'delivered' && o.seller_rating && (
                <span style={{ background:'#FEF3C7', color:'#92400E', borderRadius:100, padding:'0.4rem 0.8rem', fontSize:'0.78rem', fontWeight:600 }}>
                  {'⭐'.repeat(o.seller_rating)} Vendeur noté
                </span>
              )}
              {['confirmed', 'shipped'].includes(o.status) && (
    ...
    {confirmOrder && (
      <div onClick={() => setConfirmOrder(null)}
        ...
        <div onClick={e => e.stopPropagation()}
          style={{ background:'#FDFAF4', borderRadius:20, padding:'2rem', maxWidth:420, width:'100%', boxShadow:'0 20px 60px rgba(0,0,0,0.25)' }}>
          <div style={{ textAlign:'center', marginBottom:'1.5rem' }}>
            <div style={{ fontSize:'3rem', marginBottom:'0.8rem' }}>📦</div>
            <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'1.5rem', color:'#1E3D0F', margin:'0 0 0.5rem' }}>
              Confirmer la réception
            </h2>
            <p style={{ color:'#5C5C50', fontSize:'0.88rem', lineHeight:1.6, margin:0 }}>
          ...
          <div style={{ background:'#E8F8EE', border:'1px solid #a8dfc0', borderRadius:12, padding:'0.9rem 1rem', marginBottom:'1.5rem', fontSize:'0.82rem', color:'#145A32' }}>
            🔒 La confirmation libèrera <strong>{Number(confirmOrder.final_price).toLocaleString()} MAD</strong> de l'escrow vers le vendeur.
          </div>
          <div style={{ display:'flex', gap:'0.8rem' }}>
            <button onClick={() => setConfirmOrder(null)}
              style={{ flex:1, background:'transparent', color:'#5C5C50', border:'1.5px solid #D9CEBC', borderRadius:100, padding:'0.75rem', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", fontSize:'0.88rem' }}>
              Annuler
            </button>
            <button onClick={() => { confirmMutation.mutate(confirmOrder.id); setConfirmOrder(null); }}
              style={{ flex:2, background:'#27AE60', color:'#fff', border:'none', borderRadius:100, padding:'0.75rem', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", fontSize:'0.88rem', fontWeight:700 }}>
    ...
    {ratingOrder && (
      <RatingModal
        ...
        onSubmit={(rating, comment) => ratingMutation.mutate({ orderId: ratingOrder.id, rating, comment })}
...
function RatingModal({ order, onClose, onSubmit, isLoading }) {
  const [rating, setRating]   = React.useState(0);
  const [hover, setHover]     = React.useState(0);
  const [comment, setComment] = React.useState('');
  const C = { forest:'#1E3D0F', leaf:'#4A7C2F', cream:'#F6F1E7', beige:'#EDE6D3', white:'#FDFAF4', muted:'#5C5C50' };
  ...
  return (
    <div onClick={onClose}
      style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:700, display:'flex', alignItems:'center', justifyContent:'center', padding:'1.5rem' }}>
      <div onClick={e => e.stopPropagation()}
        style={{ background:C.white, borderRadius:24, padding:'2rem', maxWidth:440, width:'100%', boxShadow:'0 20px 60px rgba(0,0,0,0.25)' }}>

        {/* Header */}
        <div style={{ textAlign:'center', marginBottom:'1.5rem' }}>
          <div style={{ fontSize:'2.5rem', marginBottom:'0.5rem' }}>⭐</div>
          <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'1.5rem', color:C.forest, margin:'0 0 0.3rem' }}>
            Évaluer le vendeur
          </h2>
          <p style={{ color:C.muted, fontSize:'0.85rem', margin:0 }}>
            {order.seller_company} · Commande {order.order_number}
          </p>
        </div>
          ...
          {[1,2,3,4,5].map(function(n) {
            return (
              <button key={n}
                ...
                style={{ fontSize:'2.2rem', background:'transparent', border:'none', cursor:'pointer', transition:'transform 0.1s', transform:(hover||rating) >= n ? 'scale(1.2)' : 'scale(1)', lineHeight:1 }}>
                {(hover || rating) >= n ? '⭐' : '☆'}
              </button>
            );
          })}
        ...
        {/* Label note */}
        <div style={{ textAlign:'center', height:20, marginBottom:'1.2rem', fontWeight:600, fontSize:'0.88rem', color:'#D97706' }}>
          {labels[hover || rating] || ''}
        </div>
        ...
        <textarea
          ...
          placeholder="Votre commentaire (optionnel) — qualité du produit, communication, délai..."
          rows={3}
          style={{ width:'100%', padding:'0.75rem 1rem', border:'1.5px solid #D9CEBC', borderRadius:12, fontSize:'0.85rem', fontFamily:"'DM Sans',sans-serif", resize:'none', boxSizing:'border-box', outline:'none', color:C.forest, background:'#F6F1E7' }}
        />

        {/* Critères rapides */}
        <div style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap', marginTop:'0.7rem', marginBottom:'1.2rem' }}>
          {['Produit conforme', 'Livraison rapide', 'Bon emballage', 'Communication claire', 'Prix juste'].map(function(tag) {
            var isSelected = comment.includes(tag);
            return (
              <button key={tag}
                onClick={() => setComment(function(prev) { return isSelected ? prev.replace(' · '+tag,'').replace(tag+' · ','').replace(tag,'').trim() : (prev ? prev+' · '+tag : tag); })}
                style={{ padding:'0.3rem 0.7rem', borderRadius:100, border:'1px solid '+(isSelected?C.leaf:'#D9CEBC'), background:isSelected?'#E8F8EE':'transparent', color:isSelected?C.leaf:C.muted, fontSize:'0.75rem', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", fontWeight:isSelected?600:400 }}>
                {isSelected ? '✓ ' : ''}{tag}
              </button>
            );
          })}
        ...
        <div style={{ display:'flex', gap:'0.8rem' }}>
          <button onClick={onClose}
            style={{ flex:1, background:'transparent', color:C.muted, border:'1.5px solid #D9CEBC', borderRadius:100, padding:'0.75rem', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", fontSize:'0.88rem' }}>
            Annuler
          </button>
          <button onClick={() => rating > 0 && onSubmit(rating, comment)}
            disabled={rating === 0 || isLoading}
            style={{ flex:2, background:rating>0?'linear-gradient(135deg,#F59E0B,#D97706)':'#D9CEBC', color:'#fff', border:'none', borderRadius:100, padding:'0.75rem', cursor:rating>0?'pointer':'not-allowed', fontFamily:"'DM Sans',sans-serif", fontSize:'0.88rem', fontWeight:700 }}>
            {isLoading ? 'Envoi...' : rating > 0 ? 'Envoyer mon avis (' + rating + '/5)' : 'Sélectionnez une note'}
          </button>
        </div>
      </div>
    </div>
